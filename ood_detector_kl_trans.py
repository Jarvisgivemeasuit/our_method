...
def train_classifier(model, classifier, optim, 
    ...
    classifier.train()
    results, targets = torch.Tensor().cuda(), torch.Tensor().cuda()
    ...
    for idx, (inp, tar) in enumerate(loader):
        ...
        bar.suffix = '({batch:2d}/{size}) | BT:{bt:.2f} | Total:{total:} | ETA:{eta:} | Loss:{loss:.4f} | Acc:{acc:.4f} | LR:{lr:.4f}'.format(
            bt=time() - st,
            batch=idx + 1,
            size=num_iter,
    ...
    bar.finish()
    auroc, fpr, aupr = get_results(results.detach().cpu().numpy(), targets.cpu().numpy())
